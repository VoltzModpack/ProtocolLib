...
/**
 * Represents a generic store of intervals and associated values. No two
 * intervals can overlap in this representation.
 * <p>
...
public abstract class AbstractIntervalTree<TKey extends Comparable<TKey>, TValue> {
	...
	public class Entry implements Map.Entry<Range<TKey>, TValue> {
		...
		Entry(EndPoint left, EndPoint right) {
			...
			if (left.key.compareTo(right.key) > 0)
				throw new IllegalArgumentException("Left key (" + left.key
						+ ") cannot be greater than the right key (" + right.key + ")");
	...
	protected class EndPoint {
		...
		// Whether or not the end-point is opening a range, closing a range or
		// both.
		public State state;
	 ...
	 * @param lowerBound - lowest value to remove.
	 * @param upperBound - highest value to remove.
	 * @param preserveDifference - whether or not to preserve the intervals that
	...
	public Set<Entry> remove(TKey lowerBound, TKey upperBound, boolean preserveDifference) {
		...
		// Remove the previous element too. A close end-point must be preceded
		// by an OPEN end-point.
	 ...
	 * <p>
	 * Overlapping intervals are not permitted. A key can only be associated
	 * with a single value.
	 * 
	...
	public void put(TKey lowerBound, TKey upperBound, TValue value) {
		// While we don't permit overlapping intervals, we'll still allow
		// overwriting existing intervals.
	 ...
	 * @return TRUE if the given key is within an interval in this tree, FALSE
	 *         otherwise.
	 */
	...
	/**
	 * Retrieves the value of the range that matches the given key, or NULL if
	 * nothing was found.
	 * 
	 * @param key - the level to read for.
	 * @return The correct amount of experience, or NULL if nothing was
	 *         recorded.
	 */
	 ...
	 * @param inclusive - whether or not to include the current point in the
	 *            search.
	 * @return The previous end point of a given given key, or NULL if not
	 *         found.
	 */
	protected EndPoint getPreviousEndPoint(TKey point, boolean inclusive) {
		if (point != null) {
			Map.Entry<TKey, EndPoint> previous = bounds
					.floorEntry(inclusive ? point : decrementKey(point));
	...
	protected EndPoint getNextEndPoint(TKey point, boolean inclusive) {
		if (point != null) {
			Map.Entry<TKey, EndPoint> next = bounds
					.ceilingEntry(inclusive ? point : incrementKey(point));
