...
public class MinecraftMethods {
		...
		if (sendPacketMethod == null) {
			...
			try {
				sendPacketMethod = FuzzyReflection.fromClass(serverHandlerClass)
						.getMethodByName("sendPacket.*");
			} catch (IllegalArgumentException e) {
				// We can't use the method below on Netty
				if (MinecraftReflection.isUsingNetty()) {
					sendPacketMethod = FuzzyReflection.fromClass(serverHandlerClass)
							.getMethodByParameters("sendPacket",
									MinecraftReflection.getPacketClass());
					return sendPacketMethod;
				}

				Map<String, Method> netServer = getMethodList(serverHandlerClass,
						MinecraftReflection.getPacketClass());
				Map<String, Method> netHandler = getMethodList(
						MinecraftReflection.getNetHandlerClass(),
						MinecraftReflection.getPacketClass());
				...
				if (netServer.size() == 1) {
					...
					throw new IllegalArgumentException(
							"Unable to find the sendPacket method in NetServerHandler/PlayerConnection.");
				}
			}
		}
		return sendPacketMethod;
	...
	public static Method getDisconnectMethod(Class<? extends Object> playerConnection) {
		try {
			...
			return FuzzyReflection.fromObject(playerConnection).getMethodByParameters("disconnect",
					String.class);
		}
	}

	/**
	 * Retrieve the handle(Packet, GenericFutureListener[]) method of network
	 * manager.
	 * <p>
	...
	/**
	 * Retrieve the packetRead(ChannelHandlerContext, Packet) method of
	 * NetworkMananger.
	 * <p>
	 * This only exists in version 1.7.2 and above.
	 * 
	...
	public static Method getNetworkManagerReadPacketMethod() {
		if (networkManagerPacketRead == null) {
			networkManagerPacketRead = FuzzyReflection
					.fromClass(MinecraftReflection.getNetworkManagerClass(), true)
					.getMethodByParameters("packetRead", ChannelHandlerContext.class,
							MinecraftReflection.getPacketClass());
	...
	private static void initializePacket() {
		...
		if (packetReadByteBuf == null || packetWriteByteBuf == null) {
			...
			Object javaProxy = enhancer.create(new Class<?>[]{ByteBuf.class},
			...
			List<Method> candidates = FuzzyReflection
					...
					.getMethodListByParameters(Void.TYPE,
							new Class<?>[]{MinecraftReflection.getPacketDataSerializerClass()});
			...
			if (packetWriteByteBuf == null)
				throw new IllegalStateException(
						"Unable to find Packet.write(PacketDataSerializer)");
		}
	}
