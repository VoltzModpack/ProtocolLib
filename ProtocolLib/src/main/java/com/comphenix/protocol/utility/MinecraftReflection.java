...
public class MinecraftReflection {
	public static final ReportType REPORT_CANNOT_FIND_MCPC_REMAPPER = new ReportType(
			"Cannot find MCPC/Cauldron remapper.");
	public static final ReportType REPORT_CANNOT_LOAD_CPC_REMAPPER = new ReportType(
			"Unable to load MCPC/Cauldron remapper.");
	public static final ReportType REPORT_NON_CRAFTBUKKIT_LIBRARY_PACKAGE = new ReportType(
			"Cannot find standard Minecraft library location. Assuming MCPC/Cauldron.");
	...
	/**
	 * Represents a regular expression that will match the version string in a
	 * package: org.bukkit.craftbukkit.v1_6_R2 -> v1_6_R2
	 */
	private static final Pattern PACKAGE_VERSION_MATCHER = Pattern
			.compile(".*\\.(v\\d+_\\d+_\\w*\\d+)");
	...
	private static ConcurrentMap<Class<?>, MethodAccessor> getBukkitEntityCache = Maps
			.newConcurrentMap();
			...
			try {
				...
				if (packageMatcher.matches()) {
					...
					if (MinecraftVersion.SCARY_UPDATE.compareTo(version) <= 0) {
						...
						packageVersion = "v" + version.getMajor() + "_" + version.getMinor()
								+ "_R1";
						System.err.println(
								"[ProtocolLib] Assuming package version: " + packageVersion);
					}
				}
				...
				if (!MINECRAFT_FULL_PACKAGE.startsWith(MINECRAFT_PREFIX_PACKAGE)) {
					...
					if (MINECRAFT_FULL_PACKAGE.equals(FORGE_ENTITY_PACKAGE)) {
						...
						MINECRAFT_FULL_PACKAGE = CachedPackage.combine(MINECRAFT_PREFIX_PACKAGE,
								packageVersion);
					} else {
					...
					String matcher = (MINECRAFT_PREFIX_PACKAGE.length() > 0
							? Pattern.quote(MINECRAFT_PREFIX_PACKAGE + ".")
							: "") + CANONICAL_REGEX;
	...
	private static void handleLibraryPackage() {
		try {
			...
			getClassSource().loadClass(
					CachedPackage.combine(MINECRAFT_LIBRARY_PACKAGE, "com.google.gson.Gson"));
	...
	public static void setMinecraftPackage(String minecraftPackage, String craftBukkitPackage) {
		...
		if (getMinecraftServerClass() == null) {
			throw new IllegalArgumentException(
					"Cannot find MinecraftServer for package " + minecraftPackage);
		}
	...
	public static Class<?> getGameProfileClass() {
		if (!isUsingNetty())
			throw new IllegalStateException(
					"GameProfile does not exist in version 1.6.4 and earlier.");
	 ...
	 *         package cache.
	 */
	private static Class<?> fallbackMethodReturn(String nmsClass, String craftClass,
			String methodName) {
		...
		try {
			...
			if (isUsingNetty()) {
				paketContract = FuzzyClassContract.newBuilder()
						.method(FuzzyMethodContract.newBuilder().parameterDerivedOf(ByteBuf.class)
								.returnTypeVoid())
						.method(FuzzyMethodContract.newBuilder()
								.parameterDerivedOf(ByteBuf.class, 0)
								.parameterExactType(byte[].class, 1).returnTypeVoid())
						.build();
			} else {
				paketContract = FuzzyClassContract.newBuilder()
						.field(FuzzyFieldContract.newBuilder().typeDerivedOf(Map.class)
								.requireModifier(Modifier.STATIC))
						.field(FuzzyFieldContract.newBuilder().typeDerivedOf(Set.class)
								.requireModifier(Modifier.STATIC))
						.method(FuzzyMethodContract.newBuilder()
								.parameterSuperOf(DataInputStream.class).returnTypeVoid())
						.build();
			}

			// Select a method with one Minecraft object parameter
			Method selected = FuzzyReflection.fromClass(getNetServerHandlerClass())
					.getMethod(FuzzyMethodContract.newBuilder().parameterMatches(paketContract, 0)
	...
	public static Class<?> getEnumProtocolClass() {
		try {
			...
			Method protocolMethod = FuzzyReflection.fromClass(getNetworkManagerClass())
					.getMethod(FuzzyMethodContract.newBuilder().parameterCount(1)
							.parameterDerivedOf(Enum.class, 0).build());
		...
		try {
			...
			try {
				Method getScoreboardDisplayName = FuzzyReflection.fromClass(getEntityClass())
						.getMethodByParameters("getScoreboardDisplayName",
								getIChatBaseComponentClass(), new Class<?>[0]);
				Class<?> baseClass = getIChatBaseComponentClass();

				for (AsmMethod method : ClassAnalyser.getDefault()
						.getMethodCalls(getScoreboardDisplayName)) {
		...
		try {
			...
			AbstractFuzzyMatcher<Class<?>> serverPingContract = FuzzyClassContract.newBuilder()
					...
					.field(FuzzyFieldContract.newBuilder()
							.typeDerivedOf(getIChatBaseComponentClass()).build())
					.build().and(getMinecraftObjectMatcher());

			return setMinecraftClass("ServerPing", FuzzyReflection.fromClass(statusServerInfo, true)
	...
	public static Class<?> getServerPingServerDataClass() {
		...
		try {
			...
			// Find a server ping object
			AbstractFuzzyMatcher<Class<?>> serverDataContract = FuzzyClassContract
					.newBuilder().constructor(FuzzyMethodContract.newBuilder()
							.parameterExactArray(String.class, int.class))
					.build().and(getMinecraftObjectMatcher());

			return setMinecraftClass("ServerPingServerData",
					getTypeFromField(serverPing, serverDataContract));
		}
	}
	...
	public static Class<?> getServerPingPlayerSampleClass() {
		...
		try {
			...
			AbstractFuzzyMatcher<Class<?>> serverPlayerContract = FuzzyClassContract.newBuilder()
					.constructor(FuzzyMethodContract.newBuilder().parameterExactArray(int.class,
							int.class))
					.field(FuzzyFieldContract.newBuilder().typeExact(GameProfile[].class)).build()
					.and(getMinecraftObjectMatcher());

			return setMinecraftClass("ServerPingPlayerSample",
					getTypeFromField(serverPing, serverPlayerContract));
		}
	}
	 ...
	 */
	private static Class<?> getTypeFromField(Class<?> clazz,
			AbstractFuzzyMatcher<Class<?>> fieldTypeMatcher) {
	...
	private static void useFallbackServer() {
		...
		Constructor<?> selected = FuzzyReflection.fromClass(getCraftBukkitClass("CraftServer"))
				.getConstructor(FuzzyMethodContract.newBuilder()
						.parameterMatches(getMinecraftObjectMatcher(), 0).parameterCount(2)
						.build());
		...
		try {
			...
			Method selected = FuzzyReflection.fromClass(getPlayerListClass())
					.getMethod(FuzzyMethodContract.newBuilder()
							...
							.parameterExactType(String.class, 1).parameterExactType(String.class, 2)
							.build());
			...
			try {
				...
				return setMinecraftClass("NetServerHandler",
						FuzzyReflection.fromClass(getEntityPlayerClass())
								.getFieldByType("playerConnection", getNetHandlerClass())
								.getType());
				...
				FuzzyClassContract playerConnection = FuzzyClassContract.newBuilder()
						...
						.constructor(FuzzyMethodContract.newBuilder().parameterCount(3)
								.parameterSuperOf(getMinecraftServerClass(), 0)
								.parameterSuperOf(getEntityPlayerClass(), 2).build())
						.method(FuzzyMethodContract.newBuilder().parameterCount(1)
								.parameterExactType(String.class).build())
						.build();

				// If not, use duck typing
				Class<?> fieldType = FuzzyReflection.fromClass(getEntityPlayerClass(), true)
						.getField(FuzzyFieldContract.newBuilder().typeMatches(playerConnection)
								.build())
						.getType();
	...
	public static Class<?> getItemStackClass() {
		try {
			...
			return setMinecraftClass("ItemStack", FuzzyReflection
					.fromClass(getCraftItemStackClass(), true).getFieldByName("handle").getType());
		}
	}
	...
	public static Class<?> getBlockClass() {
		try {
			...
			// Useful constructors
			Method selected = reflect.getMethod(FuzzyMethodContract.newBuilder()
	...
	public static Class<?> getWorldTypeClass() {
		try {
			...
			Method selected = FuzzyReflection.fromClass(getMinecraftServerClass(), true)
					.getMethod(FuzzyMethodContract.newBuilder().parameterExactType(String.class, 0)
							.parameterExactType(String.class, 1)
							.parameterMatches(getMinecraftObjectMatcher())
							.parameterExactType(String.class, 4).parameterCount(5).build());
	...
	public static Class<?> getDataWatcherClass() {
		try {
			...
			FuzzyClassContract dataWatcherContract = FuzzyClassContract.newBuilder()
					.field(FuzzyFieldContract.newBuilder().requireModifier(Modifier.STATIC)
							.typeDerivedOf(Map.class))
					.field(FuzzyFieldContract.newBuilder().banModifier(Modifier.STATIC)
							.typeDerivedOf(Map.class))
					.method(FuzzyMethodContract.newBuilder().parameterExactType(int.class)
							.parameterExactType(Object.class).returnTypeVoid())
					.build();
			FuzzyFieldContract fieldContract = FuzzyFieldContract.newBuilder()
					.typeMatches(dataWatcherContract).build();

			// Get such a field and save the result
			return setMinecraftClass("DataWatcher", FuzzyReflection
					.fromClass(getEntityClass(), true).getField(fieldContract).getType());
		}
	}
	...
	public static Class<?> getChunkPositionClass() {
		try {
			...
			FuzzyMethodContract selected = FuzzyMethodContract.newBuilder()
					...
					.parameterExactType(int.class, 3).parameterExactType(int.class, 4).build();

			return setMinecraftClass("ChunkPosition", FuzzyReflection
					.fromClass(normalChunkGenerator).getMethod(selected).getReturnType());
		}
	}
	...
	public static Class<?> getChunkCoordIntPair() {
		...
		try {
			...
			Class<?> packet = PacketRegistry
					.getPacketClassFromType(PacketType.Play.Server.MULTI_BLOCK_CHANGE);

			AbstractFuzzyMatcher<Class<?>> chunkCoordIntContract = FuzzyClassContract.newBuilder()
					.field(FuzzyFieldContract.newBuilder().typeDerivedOf(int.class))
					.field(FuzzyFieldContract.newBuilder().typeDerivedOf(int.class))
					.method(FuzzyMethodContract.newBuilder().parameterExactArray(int.class)
							.returnDerivedOf(getChunkPositionClass()))
					.build().and(getMinecraftObjectMatcher());

			Field field = FuzzyReflection.fromClass(packet, true)
					.getField(FuzzyFieldContract.matchType(chunkCoordIntContract));
		...
		try {
			...
			Method selected = FuzzyReflection.fromClass(getDataWatcherClass(), true)
					.getMethod(FuzzyMethodContract.newBuilder().requireModifier(Modifier.STATIC)
							.parameterDerivedOf(isUsingNetty()
		...
		try {
			...
			FuzzyClassContract.Builder serverConnectionContract = FuzzyClassContract.newBuilder()
					.constructor(FuzzyMethodContract.newBuilder()
							.parameterExactType(getMinecraftServerClass()).parameterCount(1));

			if (isUsingNetty()) {
				serverConnectionContract.method(
						FuzzyMethodContract.newBuilder().parameterDerivedOf(InetAddress.class, 0)
								.parameterDerivedOf(int.class, 1).parameterCount(2));

				selected = FuzzyReflection.fromClass(getMinecraftServerClass())
						.getMethod(FuzzyMethodContract.newBuilder().requireModifier(Modifier.PUBLIC)
								.returnTypeMatches(serverConnectionContract.build()).build());

			} else {
				serverConnectionContract.method(FuzzyMethodContract.newBuilder()
		...
		try {
			...
			if (isUsingNetty()) {
				FuzzyClassContract tagCompoundContract = FuzzyClassContract.newBuilder()
						.field(FuzzyFieldContract.newBuilder().typeDerivedOf(Map.class))
						.method(FuzzyMethodContract.newBuilder()
								.parameterDerivedOf(DataOutput.class).parameterCount(1))
						.build();

				Method selected = FuzzyReflection.fromClass(getPacketDataSerializerClass())
						.getMethod(FuzzyMethodContract.newBuilder().banModifier(Modifier.STATIC)
								.parameterCount(1).parameterMatches(tagCompoundContract)
				...
				FuzzyClassContract tagCompoundContract = FuzzyClassContract.newBuilder()
						.constructor(FuzzyMethodContract.newBuilder()
								.parameterExactType(String.class).parameterCount(1))
				...
				Method selected = FuzzyReflection.fromClass(getPacketClass())
						.getMethod(FuzzyMethodContract.newBuilder().requireModifier(Modifier.STATIC)
								.parameterSuperOf(DataInputStream.class).parameterCount(1)
		...
		try {
			...
			FuzzyClassContract entityTrackerContract = FuzzyClassContract.newBuilder()
					...
					.method(FuzzyMethodContract.newBuilder()
							...
							.parameterCount(1).returnTypeVoid())
					.method(FuzzyMethodContract.newBuilder()
							.parameterSuperOf(MinecraftReflection.getEntityClass(), 0)
							.parameterSuperOf(int.class, 1).parameterSuperOf(int.class, 2)
							.parameterCount(3).returnTypeVoid())
					.build();

			Field selected = FuzzyReflection
					.fromClass(MinecraftReflection.getWorldServerClass(), true)
					.getField(FuzzyFieldContract.newBuilder().typeMatches(entityTrackerContract)
							.build());
	...
	public static Class<?> getNetworkListenThreadClass() {
		try {
			...
			FuzzyClassContract networkListenContract = FuzzyClassContract.newBuilder()
					...
					.field(FuzzyFieldContract.newBuilder().typeDerivedOf(Thread.class))
					.field(FuzzyFieldContract.newBuilder().typeDerivedOf(List.class))
					.method(FuzzyMethodContract.newBuilder()
							.parameterExactType(getNetServerHandlerClass()))
					.build();

			Field selected = FuzzyReflection
					.fromClass(MinecraftReflection.getMinecraftServerClass(), true)
					.getField(FuzzyFieldContract.newBuilder().typeMatches(networkListenContract)
							.build());
	...
	public static Class<?> getAttributeSnapshotClass() {
		try {
			...
			final Class<?> packetUpdateAttributes = PacketRegistry
					.getPacketClassFromType(PacketType.Play.Server.UPDATE_ATTRIBUTES, true);
			...
			try {
									...
									if (opcode == Opcodes.INVOKESPECIAL && isConstructor(name)) {
										...
										if (desc.startsWith("(L" + packetSignature)) {
											setMinecraftClass("AttributeSnapshot",
												...
												.startsWith("(Ljava/util/UUID;Ljava/lang/String")) {
				...
				throw new RuntimeException(
						"Unable to read the content of Packet44UpdateAttributes.", e1);
			}
	...
	public static Class<?> getIntHashMapClass() {
		try {
			...
			// add(int key, Object value)
					method(FuzzyMethodContract.newBuilder().parameterCount(2)
							.parameterExactType(int.class, 0).parameterExactType(Object.class, 1)
							.requirePublic())
					.
					// Object get(int key)
					method(FuzzyMethodContract.newBuilder().parameterCount(1)
							.parameterExactType(int.class).returnTypeExact(Object.class)
							.requirePublic())
					.
			...
			final AbstractFuzzyMatcher<Field> intHashField = FuzzyFieldContract.newBuilder()
					.typeMatches(getMinecraftObjectMatcher().and(intHashContract)).build();

			// Use the type of the first field that matches
			return setMinecraftClass("IntHashMap",
					FuzzyReflection.fromClass(parent).getField(intHashField).getType());
		}
	}
	...
	public static Class<?> getMobEffectClass() {
		try {
			...
			Class<?> packet = PacketRegistry
					.getPacketClassFromType(PacketType.Play.Server.ENTITY_EFFECT);
			Constructor<?> constructor = FuzzyReflection.fromClass(packet)
					.getConstructor(FuzzyMethodContract.newBuilder().parameterCount(2)
	...
	public static Class<?> getPacketDataSerializerClass() {
		try {
			...
			Method method = FuzzyReflection.fromClass(packet)
					.getMethod(FuzzyMethodContract.newBuilder().parameterCount(1)
							.parameterDerivedOf(ByteBuf.class).returnTypeVoid().build());
	...
	public static Class<?> getNbtCompressedStreamToolsClass() {
		try {
			...
			try {
				// Now -- we inspect all the method calls within that method,
				// and use the first foreign Minecraft class
	...
	public static Class<?> getMinecraftGsonClass() {
		try {
			...
			Class<?> match = FuzzyReflection
					.fromClass(PacketType.Status.Server.OUT_SERVER_INFO.getPacketClass())
	...
	public static ItemStack getBukkitItemStack(ItemStack bukkitItemStack) {
		...
		if (craftBukkitConstructor == null) {
			try {
				...
				throw new RuntimeException(
						"Cannot find CraftItemStack(org.bukkit.inventory.ItemStack).", e);
			}
		}
	...
	private static ItemStack getBukkitItemByMethod(ItemStack bukkitItemStack) {
		if (craftBukkitNMS == null) {
			try {
				...
				craftBukkitOBC = getCraftItemStackClass().getMethod("asCraftMirror",
	...
	public static ItemStack getBukkitItemStack(Object minecraftItemStack) {
		...
		if (craftNMSConstructor == null) {
			try {
				craftNMSConstructor = getCraftItemStackClass()
						.getConstructor(minecraftItemStack.getClass());
			} catch (Exception e) {
				// Give it a try
				if (!craftItemStackFailed)
					return getBukkitItemByMethod(minecraftItemStack);
				...
				throw new RuntimeException(
						"Cannot find CraftItemStack(net.minecraft.server.ItemStack).", e);
			}
		}
	...
	private static ItemStack getBukkitItemByMethod(Object minecraftItemStack) {
		if (craftNMSMethod == null) {
			try {
				craftNMSMethod = getCraftItemStackClass().getMethod("asCraftMirror",
						minecraftItemStack.getClass());
			} catch (Exception e) {
				craftItemStackFailed = true;
				throw new RuntimeException(
						"Cannot find CraftItemStack.asCraftMirror(net.minecraft.server.ItemStack).",
						e);
			}
		}
		...
		if (classSource == null) {
			...
			try {
				...
				if (e.getReason() != Reason.MCPC_NOT_PRESENT)
					reporter.reportWarning(MinecraftReflection.class,
							Report.newBuilder(REPORT_CANNOT_FIND_MCPC_REMAPPER));
			} catch (Exception e) {
				reporter.reportWarning(MinecraftReflection.class,
						Report.newBuilder(REPORT_CANNOT_LOAD_CPC_REMAPPER));
			}
	...
	public static Class<?> getMinecraftClass(String className, String... aliases) {
		try {
			...
			if (success != null) {
				...
				// Hack failed
				throw new RuntimeException(String.format("Unable to find %s (%s)", className,
