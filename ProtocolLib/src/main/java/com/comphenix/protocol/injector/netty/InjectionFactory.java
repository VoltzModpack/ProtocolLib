 ...
 * <p>
 * Note that the factory will return {@link ClosedInjector} when the factory is
 * closed.
 * 
 * @author Kristian
 */
class InjectionFactory {
	...
	private final ConcurrentMap<Player, Injector> playerLookup = new MapMaker().weakKeys()
			.weakValues().makeMap();
	private final ConcurrentMap<String, Injector> nameLookup = new MapMaker().weakValues()
			.makeMap();
	 ...
	 * @return A new injector, an existing injector associated with this player,
	 *         or a closed injector.
	 */
	@Nonnull
	public Injector fromPlayer(Player player, ChannelListener listener) {
		...
		injector = (ChannelInjector) ChannelInjector.findChannelHandler(channel,
				ChannelInjector.class);
	 ...
	 * <p>
	 * The injector may be NULL if the plugin has been reloaded during a player
	 * login.
	 * 
	 * @param address - the name.
	 * @return The cached injector, or a closed injector if it could not be
	 *         found.
	 */
	 ...
	 */
	@Nonnull
	public Injector fromChannel(Channel channel, ChannelListener listener,
		...
		ChannelInjector injector = new ChannelInjector(temporaryPlayer, networkManager, channel,
				listener, this);

		// Initialize temporary player
		TemporaryPlayerFactory.setInjectorInPlayer(temporaryPlayer,
				new ChannelSocketInjector(injector));
		return injector;
	}
	...
	private Object findNetworkManager(Channel channel) {
		...
		Object networkManager = ChannelInjector.findChannelHandler(channel,
	...
	/**
	 * Close all injectors created by this factory, and cease the creation of
	 * new injections.
	 */
