...
class ChannelInjector extends ByteToMessageDecoder implements Injector {
	...
	/**
	 * A flag set by the main thread to indiciate that a packet should not be
	 * processed.
	 */
	 ...
	 */
	public ChannelInjector(Player player, Object networkManager, Channel channel,
			ChannelListener channelListener, InjectionFactory factory) {
		...
		this.networkManager = Preconditions.checkNotNull(networkManager,
				"networkMananger cannot be NULL");
		this.originalChannel = Preconditions.checkNotNull(channel, "channel cannot be NULL");
		this.channelListener = Preconditions.checkNotNull(channelListener,
		...
		this.channelField = new VolatileField(FuzzyReflection.fromObject(networkManager, true)
	...
	public boolean inject() {
		synchronized (networkManager) {
			...
			// Main thread? We should synchronize with the channel thread,
			// otherwise we might see a
			// pipeline with only some of the handlers removed
			if (Bukkit.isPrimaryThread()) {
				// Just like in the close() method, we'll avoid blocking the
				// main thread
			...
			vanillaEncoder = (MessageToByteEncoder<Object>) originalChannel.pipeline()
					.get("encoder");
			...
			// Insert our handlers - note that we effectively replace the
			// vanilla encoder/decoder
			originalChannel.pipeline().addBefore("decoder", "protocol_lib_decoder", this);
			originalChannel.pipeline().addBefore("protocol_lib_decoder", "protocol_lib_finish",
					finishHandler);
			originalChannel.pipeline().addAfter("encoder", "protocol_lib_encoder", protocolEncoder);

			// Intercept all write methods
			channelField.setValue(
					new ChannelProxy(originalChannel, MinecraftReflection.getPacketClass()) {
						@Override
						protected <T> Callable<T> onMessageScheduled(final Callable<T> callable,
							...
							return new Callable<T>() {
								...
								public T call() throws Exception {
									...
									// This field must only be updated in the
									// pipeline thread
									currentEvent = event;
						...
						@Override
						protected Runnable onMessageScheduled(final Runnable runnable,
								FieldAccessor packetAccessor) {
						...
						protected PacketEvent handleScheduled(Object instance,
								FieldAccessor accessor) {
							// Let the filters handle this packet
							Object original = accessor.get(instance);

							// See if we've been instructed not to process
							// packets
	...
	private void patchEncoder(MessageToByteEncoder<Object> encoder) {
		...
		ENCODER_TYPE_MATCHER.set(encoder,
				TypeParameterMatcher.get(MinecraftReflection.getPacketClass()));
	}
	...
	/**
	 * Encode a packet to a byte buffer, taking over for the standard Minecraft
	 * encoder.
	 * 
	...
	protected void encode(ChannelHandlerContext ctx, Object packet, ByteBuf output)
		...
		try {
			...
			channelListener.getReporter().reportDetailed(this,
					Report.newBuilder(REPORT_CANNOT_INTERCEPT_SERVER_PACKET).callerParam(packet)
							.error(e).build());
		} finally {
		...
		try {
			...
			for (ListIterator<Object> it = packets.listIterator(); it.hasNext();) {
				...
				if (channelListener.includeBuffer(packetClass)) {
					...
					marker = new NettyNetworkMarker(ConnectionSide.CLIENT_SIDE,
							getBytes(byteBuffer));
				}
			...
			channelListener.getReporter().reportDetailed(this,
					Report.newBuilder(REPORT_CANNOT_INTERCEPT_CLIENT_PACKET).callerParam(byteBuffer)
							.error(e).build());
		}
	}
	...
	private void invokeSendPacket(Object packet) {
		// Attempt to send the packet with NetworkMarker.handle(), or the
		// PlayerConnection if its active
		try {
	...
	public void recieveClientPacket(final Object packet) {
		...
		Runnable action = new Runnable() {
			...
			public void run() {
				try {
					MinecraftMethods.getNetworkManagerReadPacketMethod().invoke(networkManager,
							null, packet);
				} catch (Exception e) {
					// Inform the user
					ProtocolLibrary.getErrorReporter().reportMinimal(factory.getPlugin(),
							"recieveClientPacket", e);
				}
			}
		};
	...
	public void close() {
		if (!closed) {
			...
			if (injected) {
				...
				// Calling remove() in the main thread will block the main
				// thread, which may lead
				// to a deadlock:
				// http://pastebin.com/L3SBVKzp
				//
				// ProtocolLib executes this close() method through a
				// PlayerQuitEvent in the main thread,
				// which has implicitly aquired a lock on SimplePluginManager
				// (see SimplePluginManager.callEvent(Event)).
				// Unfortunately, the remove() method will schedule the removal
				// on one of the Netty worker threads if
				// it's called from a different thread, blocking until the
				// removal has been confirmed.
				//
				// This is bad enough (Rule #1: Don't block the main thread),
				// but the real trouble starts if the same
				// worker thread happens to be handling a server ping connection
				// when this removal task is scheduled.
				// In that case, it may attempt to invoke an asynchronous
				// ServerPingEvent (see PacketStatusListener)
				// using SimplePluginManager.callEvent(). But, since this has
				// already been locked by the main thread,
				// we end up with a deadlock. The main thread is waiting for the
				// worker thread to process the task, and
				// the worker thread is waiting for the main thread to finish
				// executing PlayerQuitEvent.
				//
				// TLDR: Concurrenty is hard.
				executeInChannelThread(new Runnable() {
					@Override
					public void run() {
						for (ChannelHandler handler : new ChannelHandler[]{ChannelInjector.this,
								finishHandler, protocolEncoder}) {
							try {
		...
		originalChannel.eventLoop().execute(new Runnable() {
			...
			public void run() {
				try {
					...
					ProtocolLibrary.getErrorReporter().reportDetailed(ChannelInjector.this, Report
							.newBuilder(REPORT_CANNOT_EXECUTE_IN_CHANNEL_THREAD).error(e).build());
				}
			}
		});
	}
	...
	/**
	 * Represents a socket injector that foreards to the current channel
	 * injector.
	 * 
	 * @author Kristian
	 */
