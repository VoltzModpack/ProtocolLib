...
class ReadPacketModifier implements MethodInterceptor {
	public static final ReportType REPORT_CANNOT_HANDLE_CLIENT_PACKET = new ReportType(
			"Cannot handle client packet.");
	...
	public ReadPacketModifier(int packetID, ProxyPacketInjector packetInjector,
			ErrorReporter reporter, boolean isReadPacketDataMethod) {
	...
	/**
	 * Set the packet instance to delegate to instead, or mark the packet as
	 * cancelled.
	 * <p>
	...
	public Object intercept(Object thisObj, Method method, Object[] args, MethodProxy proxy)
		...
		if (isReadPacketDataMethod && packetInjector.requireInputBuffers(packetID)) {
			CaptureInputStream captured = new CaptureInputStream(input,
		...
		if (isReadPacketDataMethod) {
			...
			try {
				...
				reporter.reportDetailed(this, Report.newBuilder(REPORT_CANNOT_HANDLE_CLIENT_PACKET)
						.callerParam(args[0]).error(e));
			}
		}
		return returnValue;
	}
