...
public class PacketContainer implements Serializable {
	...
	private static final AggregateCloner DEEP_CLONER = AggregateCloner.newBuilder()
			.instanceProvider(DefaultInstances.DEFAULT).andThen(BukkitCloner.class)
			.andThen(ImmutableDetector.class).andThen(CollectionCloner.class)
	...
	private static final AggregateCloner SHALLOW_CLONER = AggregateCloner.newBuilder()
			...
			.andThen(new Function<BuilderParameters, Cloner>() {
				...
				public Cloner apply(@Nullable BuilderParameters param) {
					...
					return new FieldCloner(param.getAggregateCloner(),
							param.getInstanceProvider()) {
						{
	 ...
	 * Creates a packet container for an existing packet.
	 * <p>
	 * Deprecated: Use {@link #PacketContainer(PacketType, Object,
	 * StructureModifier))} instead.
	 * 
	 * @param id - ID of the given packet.
	 * @param handle - contained packet.
	...
	/**
	 * Retrieve a serializer for reading and writing ItemStacks stored in a byte
	 * array.
	 * 
	 ...
	 * <p>
	 * This modifier will automatically marshall between the Bukkit ItemStack
	 * and the internal Minecraft ItemStack.
	 * 
	 ...
	 * <p>
	 * This modifier will automatically marshall between the Bukkit world type
	 * and the internal Minecraft world type.
	 * 
	 ...
	 * <p>
	 * Note that entities are transmitted by integer ID, and the type may not be
	 * enough to distinguish between entities and other values. Thus, this
	 * structure modifier MAY return null or invalid entities for certain
	 * fields. Using the correct index is essential.
	 * 
	...
	public StructureModifier<Entity> getEntityModifier(@Nonnull World world) {
		...
		// Convert to and from the Bukkit wrapper
		return structureModifier.<Entity> withType(int.class,
				BukkitConverters.getEntityConverter(world));
	}
	...
	public StructureModifier<ChunkPosition> getPositionModifier() {
		// Convert to and from the Bukkit wrapper
		return structureModifier.withType(MinecraftReflection.getChunkPositionClass(),
	...
	public StructureModifier<ChunkCoordIntPair> getChunkCoordIntPairs() {
		...
		return structureModifier.withType(MinecraftReflection.getChunkCoordIntPair(),
	...
	public StructureModifier<NbtBase<?>> getNbtModifier() {
		// Allow access to the NBT class in packet 130
		return structureModifier.withType(MinecraftReflection.getNBTBaseClass(),
	 ...
	 * <p>
	 * This modifier will automatically marshall between the visible ProtocolLib
	 * WrappedAttribute and the internal Minecraft AttributeSnapshot.
	 * 
	...
	public StructureModifier<List<WrappedAttribute>> getAttributeCollectionModifier() {
		...
		return structureModifier.withType(Collection.class,
				BukkitConverters.getListConverter(MinecraftReflection.getAttributeSnapshotClass(),
	 ...
	 * <p>
	 * This modifier will automatically marshall between the visible ProtocolLib
	 * ChunkPosition and the internal Minecraft ChunkPosition.
	 * 
	 ...
	 * <p>
	 * This modifier will automatically marshall between the visible
	 * WrappedWatchableObject and the internal Minecraft WatchableObject.
	 * 
	...
	public StructureModifier<List<WrappedWatchableObject>> getWatchableCollectionModifier() {
		// Convert to and from the ProtocolLib wrapper
		return structureModifier.withType(Collection.class,
				BukkitConverters.getListConverter(MinecraftReflection.getWatchableObjectClass(),
	 ...
	 * <p>
	 * This modifier will automatically marshall between WrappedGameProfile and
	 * the internal Minecraft GameProfile.
	 * 
	 * @return A modifier for GameProfile fields.
	 */
	public StructureModifier<WrappedGameProfile> getGameProfiles() {
		// Convert to and from the Bukkit wrapper
		return structureModifier.<WrappedGameProfile> withType(GameProfile.class,
	 ...
	 * <p>
	 * This modifier will automatically marshall between WrappedChatComponent
	 * and the internal Minecraft GameProfile.
	 * 
	...
	/**
	 * Retrieve a read/write structure for the ServerPing fields in the
	 * following packet: <br>
	 * <ul>
	...
	public StructureModifier<Protocol> getProtocols() {
		// Convert to and from the wrapper
		return structureModifier.<Protocol> withType(EnumWrappers.getProtocolClass(),
	...
	public StructureModifier<ClientCommand> getClientCommands() {
		// Convert to and from the wrapper
		return structureModifier.<ClientCommand> withType(EnumWrappers.getClientCommandClass(),
	...
	public StructureModifier<ChatVisibility> getChatVisibilities() {
		// Convert to and from the wrapper
		return structureModifier.<ChatVisibility> withType(EnumWrappers.getChatVisibilityClass(),
	...
	public StructureModifier<Difficulty> getDifficulties() {
		// Convert to and from the wrapper
		return structureModifier.<Difficulty> withType(EnumWrappers.getDifficultyClass(),
	...
	public StructureModifier<EntityUseAction> getEntityUseActions() {
		// Convert to and from the wrapper
		return structureModifier.<EntityUseAction> withType(EnumWrappers.getEntityUseActionClass(),
	...
	public StructureModifier<NativeGameMode> getGameModes() {
		// Convert to and from the wrapper
		return structureModifier.<NativeGameMode> withType(EnumWrappers.getGameModeClass(),
	 ...
	 * <p>
	 * Note that the inflated buffers in packet 51 and 56 will be copied
	...
	public PacketContainer deepClone() {
		...
		// Fall back on the alternative (but slower) method of reading and
		// writing back the packet
	...
	// To save space, we'll skip copying the inflated buffers in packet 51 and
	// 56
				...
				return new FieldCloner(param.getAggregateCloner(), param.getInstanceProvider()) {
					{
						this.writer = new ObjectWriter() {
							protected void transformField(StructureModifier<Object> modifierSource,
								...
								if (modifierSource.getField(fieldIndex).getName()
										.startsWith("inflatedBuffer"))
									modifierDest.write(fieldIndex, modifierSource.read(fieldIndex));
								else
									defaultTransform(modifierSource, modifierDest,
											getDefaultCloner(), fieldIndex);
							};
						};
					}
				};
				...
				if (MinecraftReflection.isUsingNetty()) {
					...
					if (input.readInt() != -1)
						throw new IllegalArgumentException(
								"Cannot load a packet from 1.7.2 in 1.6.4.");
	...
	private ByteBuf createPacketBuffer() {
		return MinecraftReflection
				.getPacketDataSerializer(UnpooledByteBufAllocator.DEFAULT.buffer());
	}
	 ...
	 */
	private Method getMethodLazily(ConcurrentMap<Class<?>, Method> lookup, Class<?> handleClass,
			String methodName, Class<?> parameterClass) {
		Method method = lookup.get(handleClass);

		// Atomic operation
		if (method == null) {
			Method initialized = FuzzyReflection.fromClass(handleClass)
					.getMethod(FuzzyMethodContract.newBuilder().parameterCount(1)
							.parameterDerivedOf(parameterClass).returnTypeVoid().build());
	...
	private static class ItemStackArrayConverter implements EquivalentConverter<ItemStack[]> {
		final EquivalentConverter<ItemStack> stackConverter = BukkitConverters
