 ...
 * <p>
 * Remember to override onPacketReceiving() and onPacketSending(), depending on
 * the ConnectionSide.
 * 
 * @author Kristian
 */
public abstract class PacketAdapter implements PacketListener {
	...
	/**
	 * Initialize a packet adapter using a collection of parameters. Use
	 * {@link #params()} to get an instance to this builder.
	 * 
	 * @param params - the parameters.
	 */
	public PacketAdapter(@Nonnull AdapterParameteters params) {
		this(checkValidity(params).plugin, params.connectionSide, params.listenerPriority,
	 ...
	 * @param types - the packet types.
	 */
	public PacketAdapter(Plugin plugin, ListenerPriority listenerPriority,
	...
	public PacketAdapter(Plugin plugin, ListenerPriority listenerPriority,
		...
		this(params(plugin, Iterables.toArray(types, PacketType.class))
				.listenerPriority(listenerPriority).options(options));
	}
	 ...
	 */
	@Deprecated
	public PacketAdapter(Plugin plugin, ConnectionSide connectionSide,
			ListenerPriority listenerPriority, Set<Integer> packets) {
		this(plugin, connectionSide, listenerPriority, GamePhase.PLAYING,
				packets.toArray(new Integer[0]));
	}

	/**
	 * Initialize a packet listener for a single connection side.
	 * <p>
	 * The game phase is used to optimize performance. A listener should only
	 * choose BOTH or LOGIN if it's absolutely necessary.
	 * <p>
	 * Deprecated: Use {@link #params()} instead.
	 * 
	 * @param plugin - the plugin that spawned this listener.
	 ...
	 */
	@Deprecated
	public PacketAdapter(Plugin plugin, ConnectionSide connectionSide, GamePhase gamePhase,
			Set<Integer> packets) {
		this(plugin, connectionSide, ListenerPriority.NORMAL, gamePhase,
				packets.toArray(new Integer[0]));
	}
	...
	@Deprecated
	public PacketAdapter(Plugin plugin, ConnectionSide connectionSide,
			ListenerPriority listenerPriority, GamePhase gamePhase, Set<Integer> packets) {
	...
	@Deprecated
	public PacketAdapter(Plugin plugin, ConnectionSide connectionSide,
			ListenerPriority listenerPriority, Integer... packets) {
	 ...
	 */
	@Deprecated
	public PacketAdapter(Plugin plugin, ConnectionSide connectionSide, ListenerOptions[] options,
			Integer... packets) {
	...
	@Deprecated
	public PacketAdapter(Plugin plugin, ConnectionSide connectionSide,
			ListenerPriority listenerPriority, GamePhase gamePhase, Integer... packets) {
	 ...
	 * choose BOTH or LOGIN if it's absolutely necessary.
	 * <p>
	 * Listener options must be specified in order for
	 * {@link NetworkMarker#getInputBuffer()} to function correctly.
	 * <p>
	 * Deprecated: Use {@link #params()} instead.
	 * 
	 * @param plugin - the plugin that spawned this listener.
	...
	@Deprecated
	public PacketAdapter(Plugin plugin, ConnectionSide connectionSide,
			ListenerPriority listenerPriority, GamePhase gamePhase, ListenerOptions[] options,
			Integer... packets) {

		this(plugin, connectionSide, listenerPriority, gamePhase, options,
				PacketRegistry.toPacketTypes(Sets.newHashSet(packets), connectionSide.getSender())
						.toArray(new PacketType[0]));
	}

	// For internal use only
	private PacketAdapter(Plugin plugin, ConnectionSide connectionSide,
			ListenerPriority listenerPriority, GamePhase gamePhase, ListenerOptions[] options,
			PacketType... packets) {
		...
		if (connectionSide.isForServer())
			sendingWhitelist = ListeningWhitelist.newBuilder().priority(listenerPriority)
					.types(packets).gamePhase(gamePhase).options(serverOptions).build();

		if (connectionSide.isForClient())
			receivingWhitelist = ListeningWhitelist.newBuilder().priority(listenerPriority)
					.types(packets).gamePhase(gamePhase).options(clientOptions).build();
	...
	public void onPacketReceiving(PacketEvent event) {
		...
		throw new IllegalStateException(
				"Override onPacketReceiving to get notifcations of received packets!");
	}
	...
	/**
	 * Retrieves the name of the plugin that has been associated with the
	 * listener.
	 * 
	...
	public String toString() {
		...
		return String.format("PacketAdapter[plugin=%s, sending=%s, receiving=%s]",
				getPluginName(this), sendingWhitelist, receivingWhitelist);
	}

	/**
	 * Construct a helper object for passing parameters to the packet adapter.
	 * <p>
	 * This is often simpler and better than passing them directly to each
	 * constructor.
	 * 
	 ...
	 * <p>
	 * This is often simpler and better than passing them directly to each
	 * constructor. Deprecated: Use {@link #params(Plugin, PacketType...)}
	 * instead.
	 * 
	 * @param plugin - the plugin that spawned this listener.
	 * @param packets - the packet IDs the listener is looking for.
	...
	/**
	 * Represents a builder for passing parameters to the packet adapter
	 * constructor.
	 * <p>
	...
	public static class AdapterParameteters {
		...
		/**
		 * Set the plugin that spawned this listener. This parameter is
		 * required.
		 * 
		 * @param plugin - the plugin.
		...
		/**
		 * Set the packet types this listener is looking for. This parameter is
		 * required.
		 * 
		...
		public AdapterParameteters connectionSide(@Nonnull ConnectionSide connectionSide) {
			this.connectionSide = Preconditions.checkNotNull(connectionSide,
					"connectionside cannot be NULL.");
			return this;
		}
		...
		/**
		 * Set the the event priority, where the execution is in ascending order
		...
		/**
		 * Set which game phase this listener is active under. This is a hint
		 * for ProtocolLib to start intercepting login packets.
		 * <p>
		 * Default is {@link GamePhase#PLAYING}, which will not intercept login
		 * packets.
		 * 
		...
		/**
		 * Set the game phase to {@link GamePhase#LOGIN}, allowing ProtocolLib
		 * to intercept login packets.
		 * 
		 * @return This builder, for chaining.
		 */
		...
		/**
		 * Set listener options that decide whether or not to intercept the raw
		 * packet data as read from the network stream.
		 * <p>
		 ...
		 * {@link ListenerOptions#INTERCEPT_INPUT_BUFFER}, causing ProtocolLib
		 * to read the raw packet data from the network stream.
		 * 
		 * @return This builder, for chaining.
		 */
		...
		/**
		 * Set the listener option to
		 * {@link ListenerOptions#DISABLE_GAMEPHASE_DETECTION}, causing
		 * ProtocolLib to ignore automatic game phase detection.
		 * <p>
		...
		/**
		 * Set the listener option to {@link ListenerOptions#ASYNC}, indicating
		 * that our listener is thread safe.
		 * <p>
		...
		public AdapterParameteters types(@Nonnull PacketType... packets) {
			...
			if (connectionSide == null) {
				for (PacketType type : packets) {
					this.connectionSide = ConnectionSide.add(this.connectionSide,
							type.getSender().toSide());
				}
			}
