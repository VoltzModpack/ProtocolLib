...
public class WrappedServerPing extends AbstractWrapper {
	 ...
	 */
	private static ImmutableMap<MinecraftVersion, Integer> VERSION_NUMBERS = ImmutableMap
			.<MinecraftVersion, Integer> builder().put(MinecraftVersion.WORLD_UPDATE, 4)
			.put(MinecraftVersion.SKIN_UPDATE, 5).build();
	...
	private static ConstructorAccessor SERVER_PING_CONSTRUCTOR = Accessors
			.getConstructorAccessor(SERVER_PING);
	private static FieldAccessor DESCRIPTION = Accessors.getFieldAccessor(SERVER_PING,
			MinecraftReflection.getIChatBaseComponentClass(), true);
	private static FieldAccessor PLAYERS = Accessors.getFieldAccessor(SERVER_PING,
			MinecraftReflection.getServerPingPlayerSampleClass(), true);
	private static FieldAccessor VERSION = Accessors.getFieldAccessor(SERVER_PING,
			MinecraftReflection.getServerPingServerDataClass(), true);
	private static FieldAccessor FAVICON = Accessors.getFieldAccessor(SERVER_PING, String.class,
			true);

	// For converting to the underlying array
	private static EquivalentConverter<Iterable<? extends WrappedGameProfile>> PROFILE_CONVERT = BukkitConverters
			.getArrayConverter(GameProfile.class,
	...
	private static ConstructorAccessor PLAYERS_CONSTRUCTOR = Accessors
			.getConstructorAccessor(PLAYERS_CLASS, int.class, int.class);
	private static FieldAccessor[] PLAYERS_INTS = Accessors.getFieldAccessorArray(PLAYERS_CLASS,
			int.class, true);
	private static FieldAccessor PLAYERS_PROFILES = Accessors.getFieldAccessor(PLAYERS_CLASS,
	...
	private static MethodAccessor GSON_TO_JSON = Accessors.getMethodAccessor(GSON_CLASS, "toJson",
			Object.class);
	private static MethodAccessor GSON_FROM_JSON = Accessors.getMethodAccessor(GSON_CLASS,
			"fromJson", String.class, Class.class);
	...
	private static ConstructorAccessor VERSION_CONSTRUCTOR = Accessors
			.getConstructorAccessor(VERSION_CLASS, String.class, int.class);
	private static FieldAccessor VERSION_NAME = Accessors.getFieldAccessor(VERSION_CLASS,
			String.class, true);
	private static FieldAccessor VERSION_PROTOCOL = Accessors.getFieldAccessor(VERSION_CLASS,
			int.class, true);
	...
	/**
	 * Construct a new server ping initialized with a zero player count, and
	 * zero maximum.
	 * <p>
	...
	protected void resetVersion() {
		...
		version = VERSION_CONSTRUCTOR.invoke(minecraftVersion.toString(),
				VERSION_NUMBERS.get(minecraftVersion));
	 ...
	 * 
	 * @param image - the new compressed image or NULL if no favicon should be
	 *            displayed.
	 */
	 ...
	 * @throws IllegalStateException If the player count has been hidden via
	 *             {@link #setPlayersVisible(boolean)}.
	 */
	 ...
	 * <p>
	 * As of 1.7.2, this is completely unrestricted, and can be both positive
	 * and negative, as well as higher than the player maximum.
	 * 
	 ...
	 * @throws IllegalStateException If the player maximum has been hidden via
	 *             {@link #setPlayersVisible(boolean)}.
	 */
	 ...
	 * <p>
	 * The 1.7.2 accepts any value as a player maximum, positive or negative. It
	 * even permits a player maximum that is less than the player count.
	 * 
	 ...
	 * <p>
	 * Note that this may set the current player count and maximum to their
	 ...
	 * 
	 * @return Logged in players or an empty list if no player names will be
	 *         displayed.
	 */
	...
	public void setPlayers(Iterable<? extends WrappedGameProfile> profile) {
		if (players == null)
			resetPlayers();
		PLAYERS_PROFILES.set(players,
				(profile != null)
						? PROFILE_CONVERT.getGeneric(GameProfile[].class, profile)
						: null);
	}
	...
	public void setBukkitPlayers(Iterable<? extends Player> players) {
		...
		for (Player player : players) {
			GameProfile profile = (GameProfile) ENTITY_HUMAN_PROFILE
					.get(BukkitUnwrapper.getInstance().unwrapItem(player));
		...
		public static CompressedImage fromBase64Png(String base64) {
			try {
				...
				throw new IllegalReferenceCountException(
						"Must be a pure base64 encoded string. Cannot be an encoded text.", e);
			}
		}
	...
	/**
	 * Represents a compressed image that starts out as an encoded base 64
	 * string.
	 * 
	 * @author Kristian
	 */
