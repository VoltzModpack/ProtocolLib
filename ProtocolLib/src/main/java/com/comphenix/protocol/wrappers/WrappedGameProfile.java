...
public class WrappedGameProfile extends AbstractWrapper {
	...
	public static final ReportType REPORT_INVALID_UUID = new ReportType(
			"Plugin %s created a profile with '%s' as an UUID.");

	// Version 1.7.2 and 1.7.8 respectively
	private static final ConstructorAccessor CREATE_STRING_STRING = Accessors
			.getConstructorAccessorOrNull(GameProfile.class, String.class, String.class);
	private static final FieldAccessor GET_UUID_STRING = Accessors
			.getFieldAcccessorOrNull(GameProfile.class, "id", String.class);
	 ...
	 * <p>
	 * Note that this constructor is very lenient when parsing UUIDs for
	 * backwards compatibility reasons. IDs that cannot be parsed as an UUID
	 * will be hashed and form a version 3 UUID instead.
	 * <p>
	...
	/**
	 * Parse an UUID using very lax rules, as specified in
	 * {@link #WrappedGameProfile(UUID, String)}.
	 * 
	 * @param id - text.
	...
	private static UUID parseUUID(String id) {
		try {
			...
			ProtocolLibrary.getErrorReporter().reportWarning(WrappedGameProfile.class,
	 ...
	 * <p>
	 * Note that Minecraft 1.7.5 and earlier doesn't use UUIDs internally, and
	 * it may not be possible to convert the string to an UUID.
	 * <p>
	 * We use the same lax conversion as in
	 * {@link #WrappedGameProfile(String, String)}.
	 * 
	 * @return The UUID, or NULL if the UUID is NULL.
	 * @throws IllegalStateException If we cannot parse the internal ID as an
	 *             UUID.
	 */
	public UUID getUUID() {
		...
		if (uuid == null) {
			try {
				...
				throw new IllegalStateException("Cannot parse ID " + getId()
						+ " as an UUID in player profile " + getName());
			}
		}
		return uuid;
	}
	 ...
	 * <p>
	 * Note that there's nothing stopping plugins from creating non-standard
	 * UUIDs.
	 * <p>
	...
	@Override
	public int hashCode() {
		// Mojang's hashCode() is broken, it doesn't handle NULL id or name. So
