...
class TileEntityAccessor<T extends BlockState> {
	...
	/**
	 * Token indicating that the given block state doesn't contany any tile
	 * entities.
	 */
	private static final TileEntityAccessor<BlockState> EMPTY_ACCESSOR = new TileEntityAccessor<BlockState>();

	/**
	 * Cached field accessors - {@link #EMPTY_ACCESSOR} represents no valid tile
	 * entity.
	 */
	private static final ConcurrentMap<Class<?>, TileEntityAccessor<?>> cachedAccessors = Maps
			.newConcurrentMap();
				...
				if (expectedDesc.equals(desc)) {
					return new EmptyMethodVisitor() {
						...
						public void visitMethodInsn(int opcode, String owner, String name,
								String desc) {
							// This must be a virtual call on NBTTagCompound
							// that accepts a String
							if (opcode == Opcodes.INVOKEVIRTUAL && tagCompoundName.equals(owner)
						...
						public void visitEnd() {
							if (readMethods > writeMethods) {
								readCompound = Accessors.getMethodAccessor(tileEntityClass,
										methodName, nbtCompoundClass);
							} else if (writeMethods > readMethods) {
								writeCompound = Accessors.getMethodAccessor(tileEntityClass,
										methodName, nbtCompoundClass);
							}
							super.visitEnd();
						}
	 ...
	 * @return The accessor, or NULL if this block state doesn't contain any
	 *         tile entities.
	 */
	...
	public static <T extends BlockState> TileEntityAccessor<T> getAccessor(T state) {
		...
		if (accessor == null) {
			...
			try {
				field = Accessors.getFieldAccessor(craftBlockState,
						MinecraftReflection.getTileEntityClass(), true);
			} catch (Exception e) {
